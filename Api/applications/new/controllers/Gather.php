...
class Gather extends CA_Controller {
    ...
    {/*{{{*/
        $sql = 'select a.category, ifnull(ROUND(avg((select Sum(cutScore) from tbl_trace where cutScore > 0 and taskId = a.id)), 1), 0) as cutScore from tbl_task a where a.unitId = '.$unitId.' group by category;';
