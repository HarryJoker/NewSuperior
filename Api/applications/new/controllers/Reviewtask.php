<?php
class Reviewtask extends CA_Controller {
...
// 12:上报领导
    ...
    public function create() {
        ...
        $reviewTask = $this->input->post('task');
        $reviewTrace = $this->input->post('trace');

        $reviewTaskId = $this->reviewtask_model->createReviewTask($reviewTask);
        $reviewTrace['reviewTaskId'] = $reviewTaskId;

        $this->load->model("reviewtrace_model");
        $this->reviewtrace_model->createReviewTrace($reviewTrace);

        // $id = $this->task_model->create_id($data);
        $this->set_content(0, '上报审核成功', array('id' => $id));
    }


    //review完成，上报到平台，删除单位内部索引
    public function delete($reviewTaskId = 0) {
        $rows = $this->reviewtask_model->update_where(array('valid' => 0), array('id' => $reviewTaskId));
        return $rows;
    }
    ...
    public function getUnitReviewTasks($unitId = 0, $category = 0) {
        $result = $this->reviewtask_model->getUnitReviewTasks($unitId, $category);
        $this->set_content(0, '获取成功', $result);
    }
