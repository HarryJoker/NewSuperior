...
import com.mm.dss.demo.PlayBackActivity;
...
public class DeviceChannelAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if(convertView == null){
            convertView = LayoutInflater.from(context).inflate(R.layout.dss_fragment_channel_list_item, null);
        ...
        if(channelInfo != null) {
            if (channelInfo.getVideoInputInfo() != null && channelInfo.getVideoInputInfo().getCameraType() == ChannelInfo.CameraType.CameraPtz) {
                holder.channelThumbnail.setImageResource(R.drawable.dss_common_channel_icon_ptz);
            } else {
                holder.channelThumbnail.setImageResource(R.drawable.dss_common_channel_icon_normal);
            }
