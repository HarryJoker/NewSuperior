...
public class GestureLockLayout extends RelativeLayout
{
	 ...
	 */
	private List<Long> mChoose = new ArrayList<Long>();
	 ...
	 * 回调获得的手势密码
	 */
	private OnGestureLockAnswer mOnGestureLockAnswer;
	...
	public GestureLockLayout(Context context, AttributeSet attrs, int defStyle)
	{
		super(context, attrs, defStyle);
		/**
		 * 获得所有自定义的参数的值
		 */
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.GestureLockLayout, defStyle, 0);
		...
		for (int i = 0; i < n; i++)
		{
			...
			switch (attr)
			{
			case R.styleable.GestureLockLayout_color_no_finger_inner_circle:
				mNoFingerInnerCircleColor = a.getColor(attr, mNoFingerInnerCircleColor);
				break;
			case R.styleable.GestureLockLayout_color_no_finger_outer_circle:
				mNoFingerOuterCircleColor = a.getColor(attr, mNoFingerOuterCircleColor);
				break;
			case R.styleable.GestureLockLayout_color_finger_on:
				mFingerOnColor = a.getColor(attr, mFingerOnColor);
				break;
			case R.styleable.GestureLockLayout_color_finger_up:
				mFingerUpColor = a.getColor(attr, mFingerUpColor);
				break;
			case R.styleable.GestureLockLayout_count:
				mCount = a.getInt(attr, 3);
				break;
			case R.styleable.GestureLockLayout_tryTimes:
...
//			mPaint.setStrokeWidth(mGestureLockViewWidth * 0.29f);
			mPaint.setStrokeWidth(mGestureLockViewWidth * 0.2f);
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		case MotionEvent.ACTION_UP:
			...
			if (mOnGestureLockAnswer != null && mChoose.size() > 0) {
				Long[] integers = new Long[mChoose.size()];//当泛型为Integer时，需要
				integers = (Long[])mChoose.toArray(integers);
				mOnGestureLockAnswer.onLockAnswer(integers);
			}
			...
			{
				int childId = mChoose.get(i).intValue();
				int nextChildId = mChoose.get(i + 1).intValue();
	...
	public void setmOnGestureLockAnswer(OnGestureLockAnswer onGestureLockAnswer) {
		this.mOnGestureLockAnswer = onGestureLockAnswer;
	}
	...
	{
		if (answer != null) this.mAnswer = answer;
	}
	...
	public interface OnGestureLockAnswer
	{
		/**
		 * 获取设置的密码
		 * @param answers
