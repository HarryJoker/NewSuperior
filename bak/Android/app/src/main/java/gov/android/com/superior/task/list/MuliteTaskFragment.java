...
/**
 *  三级折叠列表：（督查，县长）
 *  只有category= 1：政府工作报告使用
 */
public class MuliteTaskFragment extends BaseFragment {
    ...
    public static MuliteTaskFragment newInstance(int category, int listType, int unitId) {
        MuliteTaskFragment fragment = new MuliteTaskFragment();
     ...
     * @return
     */
    public static MuliteTaskFragment newInstance(int category, int listType, int unitId, int status) {
        MuliteTaskFragment fragment = new MuliteTaskFragment();
        Bundle args = new Bundle();
        args.putInt(KEY_CATEGORY, category);
        args.putInt(KEY_TYPE, listType);
    ...
    public static MuliteTaskFragment newInstance(int category, int listType, int unitId, int status, int month) {
        MuliteTaskFragment fragment = new MuliteTaskFragment();
        Bundle args = new Bundle();
        args.putInt(KEY_CATEGORY, category);
        args.putInt(KEY_TYPE, listType);
        ...
        public void onSuccess(Response<JSONArray> response) {
            ...
            if (response.body() != null) {
                if (response.body().size() > 0) {
                    rc_muilte_task.setAdapter(new MuliteTaskAdapter(getActivity(), listType, response.body()));
                } else {
